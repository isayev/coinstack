...
class CRROService(CatalogService):
    ...
    def normalize_reference(self, raw: str) -> str | None:
        ...
        if subtype:
            return f"crawford.{number}.{subtype}".lower()
        return f"crawford.{number}".lower()
    ...
    def parse_reference(self, raw: str) -> dict | None:
        ...
        # Pattern with explicit prefix: Crawford 335/1c, Cr. 335/1, RRC 335/1c
        pattern_prefix = r"(?:Crawford|Cr\.?|RRC)\s*(\d+)/(\d+)([a-z]?)?"
        match = re.match(pattern_prefix, raw, re.IGNORECASE)
        ...
        # Pattern without prefix (just number): 335/1c
        pattern_bare = r"^(\d+)/(\d+)([a-z]?)?$"
        match = re.match(pattern_bare, raw)
    ...
    async def build_reconcile_query(
        ...
        if parsed:
            query_str = f"Crawford {parsed['number']}"
            if parsed.get("subtype"):
                query_str += parsed["subtype"]
        else:
            query_str = ref
    ...
    def parse_payload(self, jsonld: dict) -> CatalogPayload:
        ...
        """
        def get_label(obj: Any) -> str | None:
