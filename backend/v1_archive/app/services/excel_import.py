...
def parse_reign_years(reign_str: str) -> tuple[Optional[int], Optional[int]]:
    ...
    # Pre-processing: handle "BC X-Y" format by moving BC to end if needed for standard regex
    # But better to just support the patterns directly
    
    # Pattern 1: "27 BC-AD 14" or "2 BC-AD 4" OR "BC 27-AD 14"
    bc_ad_match = re.search(r"(?:BC\s*)?(\d+)\s*BC\s*[-–]\s*AD\s*(\d+)|BC\s*(\d+)\s*[-–]\s*AD\s*(\d+)", reign_str, re.IGNORECASE)
    if bc_ad_match:
        # Check which group matched
        if bc_ad_match.group(1):
            start = -int(bc_ad_match.group(1))
            end = int(bc_ad_match.group(2))
        else:
            start = -int(bc_ad_match.group(3))
            end = int(bc_ad_match.group(4))
        return start, end

    # Pattern 2: "BC 27-14" or "27-14 BC" (Range entirely in BC)
    # Check for BC indication anywhere in string then parse numbers
    # Be careful not to match AD ranges
    
    # Explicit BC range patterns
    bc_range_match = re.search(r"(?:BC\s*)?(\d+)\s*[-–]\s*(\d+)\s*BC|BC\s*(\d+)\s*[-–]\s*(\d+)", reign_str, re.IGNORECASE)
    if bc_range_match and "AD" not in reign_str:
        if bc_range_match.group(1):
             # "27-14 BC" -> usually start is larger number (older), end is smaller (younger)
             # But sometimes written "14-27 BC" (less common but possible)
             # Let's assume standard descending order for BC if numbers are reversed? 
             # Actually, usually ranges are start-end. For BC, start is smaller negative (larger abs), end is larger negative (smaller abs).
             # "27-14 BC" -> -27 to -14.
             val1 = int(bc_range_match.group(1))
             val2 = int(bc_range_match.group(2))
        else:
             # "BC 27-14"
             val1 = int(bc_range_match.group(3))
             val2 = int(bc_range_match.group(4))
        
        # Determine strict start/end. Start should be < End (arithmetically).
        # But commonly "27-14 BC" means from year 27 BC to 14 BC.
        # Year -27 < Year -14.
        start = -max(val1, val2) 
        end = -min(val1, val2)
        return start, end

    # Pattern 3: "AD 14-37" or "AD 14 - 37" or "AD 140/141" (range with hyphen, en-dash, or slash)
    ...
    # Pattern 4: "96 BC" or "54 BC" or "BC 96" (single BC year)
    single_bc_match = re.search(r"(\d+)\s*BC|BC\s*(\d+)", reign_str, re.IGNORECASE)
    if single_bc_match:
        val = single_bc_match.group(1) or single_bc_match.group(2)
        year = -int(val)  # BC is negative
        return year, year
    
    # Pattern 5: "AD 75" (single AD year)
    single_ad_match = re.search(r"AD\s*(\d+)", reign_str, re.IGNORECASE)
    if single_ad_match:
        year = int(single_ad_match.group(1))
        return year, year
    
    # Pattern 6: Plain range "14-37" or "140/141" (assume AD if no BC/AD present)
    ...
    # Pattern 7: Single year without BC/AD (assume AD)
