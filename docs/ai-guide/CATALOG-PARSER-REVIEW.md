...
- **RIC**: IV.1 / IV-1 / IV/1 → same canonical; V.II 325 (Roman part) → volume V.2 for canonical/OCRE; RIC I (2) 207 / RIC I(2) 207 (parenthesized edition) → volume I.2; RIC III, 303 and RIC III, 303 - Antoninus Pius (comma form; trailing " - ruler" ignored) → III, 303; edition ²/³ → volume .2/.3; unknown mint → title case.
...
- **OCRE query**: When volume has a dot (volume.part, e.g. V.2 from RIC V.II 325), OCRE `build_reconcile_query` sends it as-is ("RIC V.2 325"); edition (2)/(3) is added only when explicitly parsed (e.g. RIC I², RIC I(2) 207)—do not default to (2); "RIC III(2) 303" was wrongly matching RIC II Part 3. When context has ruler/authority (e.g. Diocletian) or trailing " - Ruler" (e.g. "RIC III, 303 - Antoninus Pius"), the query uses authority so "RIC III Antoninus Pius 303" matches. Quick Lookup (IdentityStep) passes issuer and mint as `coinContext` so "RIC III 303" with Antoninus Pius selected uses ruler in the OCRE query. See `tests/unit/infrastructure/services/test_ocre.py`.
