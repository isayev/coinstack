...
class SlabGrade(BaseModel):
    ...
    numeric_grade: Optional[int] = None  # 1-70 scale (US coins, some ancients)
...
class PhysicalData(BaseModel):
    ...
    def weight_g(self) -> Optional[float]:
        """Alias for compatibility with CNG format"""
...
class AuctionInfo(BaseModel):
    ...
    auction_type: HeritageAuctionType
    lot_number: int
...
class HeritageSearchResult(BaseModel):
    ...
    current_bid_usd: Optional[int] = None
    sold_price_usd: Optional[int] = None
    is_sold: bool = False
...
class HeritageSearchResults(BaseModel):
    """Search results page"""
    query: str
    total_results: int
    page: int
    results: list[HeritageSearchResult]
    has_more: bool
